...
main PROC
    ...
    add eax, ebx        ; A = A + B = 4 + 3 = 7
    add ecx, edx        ; C = C + D = 2 + 1 = 3
    sub eax, ecx        ; A = A - C = 7 - 3 = 4
...
.data
    b BYTE 255                  ; 8-bit unsigned
    sb SBYTE -128               ; 8-bit signed
    w WORD 65535                ; 16-bit unsigned
    sw SWORD -32768             ; 16-bit signed
    dw DWORD 4294967295         ; 32-bit unsigned
    sdw SDWORD -2147483648      ; 32-bit signed
    qw QWORD 18446744073709551615  ; 64-bit unsigned
...
> 답: B8은 mov, 00000005은 5, 83은 add, 89는 mov, B9는 두번째 mov?, E8은 call

```asm
00000000                     .data
00000000 00000000            sum DWORD 00000000h

00000000                     .code
00000000                     main PROC
00000000  B8 00000005            mov eax, 5
00000005  83 C0 06               add eax, 6
00000008  89 05 00000000 R       mov sum, eax
0000000E  B9 00000000            mov ecx, 0
...
> 답: 64비트 변수로 바꾸면 레지스터도 64비트 레지스터로 변경해야 함. 그런데 32비트 환경에서는 64비트 레지스터를 인식하지 못함. 그래서 64비트 환경으로 변경. .386 등을 제거함. Windows API 호출 규약을 변경해야 한다.

```asm
ExitProcess PROTO

.data
    firstval  QWORD 2000200020002000h    ; 64비트 정수
...
.code
main PROC
    mov rax, firstval        ; 64비트 RAX 레지스터 사용
    add rax, secondval
    add rax, thirdval
    mov sum, rax

    mov rcx, 0               ; 64비트 Windows ABI: 첫 번째 인자는 RCX
    call ExitProcess         ; 정식 API 호출
main ENDP
END
```
